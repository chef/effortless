...
Describe "Chef 19 integration: scaffolding-chef-infra with chef-infra-client from unstable" {
    ...
    Context "Integration: Chef 19 successfully executes the run from the run hook" {
        It "File created by the cookbook should exist" {
            Test-Path -LiteralPath "c:/temp/test-integration" | Should Be $true
        }

        It "File created by the cookbook should have 'Hello from Chef 19!' in it" {
            $TestFileContent = Get-Content "c:/temp/test-integration"
    ...
    Context "Build: client-config.rb pkg_svc_data_path renders" {
        It "client-config.rb renders" {
            $config = Get-Content "C:\hab\svc\user-windows-integration\config\client-config.rb" | Select-String -Pattern 'cache_path'
            $config = $config -split ' '
            $config[1] | Should be "'C:\hab\svc\user-windows-integration\data/cache'"
        }
    }

    Context "API: scaffold_cacerts matches run hook core/cacerts" {
        It "SSL_CERT_FILE should be core/cacerts" {
            $cert_file = Get-Content "C:\hab\svc\user-windows-integration\hooks\run" | Select-String -Pattern '\$env:SSL_CERT_FILE'
        ...
        It "SSL_CERT_DIR should be core/cacerts" {
            $cert_dir = Get-Content "C:\hab\svc\user-windows-integration\hooks\run" | Select-String -Pattern '\$env:SSL_CERT_DIR'
    ...
    Context "API: scaffold_chef_client matches run hook chef/chef-infra-client" {
        It "The chef-client should be chef/chef-infra-client" {
            $chef_client_pkg = Get-Content "C:\hab\svc\user-windows-integration\hooks\run" | Select-String -Pattern '\w+/bin/chef-client.bat -z'
